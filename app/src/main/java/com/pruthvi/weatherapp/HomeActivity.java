...
import static com.pruthvi.weatherapp.location.CityFinder.getCityNameUsingNetwork;
import static com.pruthvi.weatherapp.location.CityFinder.setLongitudeLatitude;
import static com.pruthvi.weatherapp.network.InternetConnectivity.isInternetConnected;
...
import com.pruthvi.weatherapp.adapter.DaysAdapter;
import com.aniketjain.weatherapp.databinding.ActivityHomeBinding;
import com.pruthvi.weatherapp.location.LocationCord;
import com.pruthvi.weatherapp.toast.Toaster;
...
public class HomeActivity extends AppCompatActivity {
    ...
    private void setLatitudeLongitudeUsingCity(String cityName) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, URL.getCity_url(), null, response -> {
            try {
                // WeatherAPI places coordinates in 'location' instead of 'coord'
                LocationCord.lat = response.getJSONObject("location").getString("lat");
                LocationCord.lon = response.getJSONObject("location").getString("lon");
    ...
    private void getTodayWeatherInfo(String name) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url.getLink(), null, response -> {
            try {
                ...
                JSONObject forecastToday = response.getJSONObject("forecast").getJSONArray("forecastday").getJSONObject(0).getJSONObject("day");
                JSONObject conditionObj = current.getJSONObject("condition");

                update_time = current.getLong("last_updated_epoch");
                updated_at = new SimpleDateFormat("EEEE hh:mm a", Locale.ENGLISH).format(new Date(update_time * 1000));

                condition = conditionObj.getInt("code");
                isDay = current.getInt("is_day");
                description = conditionObj.getString("text");

                temperature = String.valueOf(Math.round(current.getDouble("temp_c")));
                min_temperature = String.format("%.0f", forecastToday.getDouble("mintemp_c"));
                max_temperature = String.format("%.0f", forecastToday.getDouble("maxtemp_c"));

                pressure = current.getString("pressure_mb");
                wind_speed = current.getString("wind_kph");
    ...
    private void updateUI() {
        ...
        binding.layout.conditionIv.setImageResource(
                getResources().getIdentifier(
                        UpdateUI.getIconID(condition, isDay),
                        "drawable",
                        getPackageName()
                ));
